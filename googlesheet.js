function loadDataSetDisplayCategoryCoes2(){
  ...
  lastDate = new Date(now.getFullYear(), now.getMonth()-1, firstDate2.getDate()+28);
  lastDate = getFormatDate(lastDate);
 console.log(lastDate);

  const coupangAPIPath = '/v2/providers/openapi/apis/api/v4/vendors/' + salesID + '/returnRequests';
  ...
  for(var i=0;i<4;i++){
    const coupangAPIQuery = '?searchType=timeFrame&createdAtFrom=' + lastDate + '&createdAtTo=' + firstDate + '&status='+queryStatus[i];
    console.log(queryStatus[i]);
     dataSet = connectAndGetAPIData(coupangAPIPath, coupangAPIQuery)
